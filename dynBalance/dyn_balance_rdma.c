...
int main()
{
  ...
  */
  ...
  // Create a shared index so jobs sync dynamically using it
  int job_index = 0;

  // Create a window over the shared index
  MPI_Win index_window;
  MPI_Win_create( &job_index , sizeof(int) , sizeof(int) , MPI_INFO_NULL , MPI_COMM_WORLD , &index_window);

  // Get initial job
  MPI_Win_lock( MPI_LOCK_SHARED , 0 , 0 , index_window);

  MPI_Fetch_and_op( &jobs_to_compute , &next_job , MPI_INT , 0 , 0 , MPI_SUM , index_window);
