...
int main(int argc, char **argv)
{
  ...
  // MPI-IO create filetype datatype
  MPI_Type_vector( s.rows , s.cols , gsize[COLS] , MPI_CHAR , &mpi_file_block_t);
...
int read_input(state * s, const char * filename, const int *gsize, parallel_state * mpi)
{
  ...
  return_val |= MPI_File_set_view( in_file, my_offset, MPI_CHAR, mpi_file_block_t, "native", MPI_INFO_NULL);
...
void print_state(state * s, const char * filename, int *gsize, parallel_state * mpi)
{
  ...
  return_val |= MPI_File_set_view( out_file, my_offset, MPI_CHAR, mpi_file_block_t, "native", MPI_INFO_NULL);
